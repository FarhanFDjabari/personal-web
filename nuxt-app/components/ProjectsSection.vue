    ...
    <div class="container mx-auto">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card
          ...
          :key="repo.githubUrl"
          class="hover-lift transition-all duration-300 overflow-hidden"
        >
          <CardHeader>
          ...
          <CardContent class="space-y-4">
            ...
            <div class="flex flex-wrap gap-2">
              <Badge v-for="tech in repo.technologies.slice(0, 5)" :key="tech" variant="secondary">
                {{ tech }}
              </Badge>
            </div>
            <div class="flex gap-2 pt-2">
              <Button as="a" :href="repo.githubUrl" target="_blank" variant="outline" size="sm" class="gap-2">
                <Icon name="lucide:github" class="w-4 h-4" />
                {{ t('projects.github') }}
              </Button>
              <Button v-if="repo.liveUrl" as="a" :href="repo.liveUrl" target="_blank" size="sm" class="gap-2">
